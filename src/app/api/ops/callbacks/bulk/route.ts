...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const results = await performAdminQueueAction({ kind: 'callback', actor: session.email, ...parsed });
    const updated = results.filter(result => result.status === 'updated');
    await logActivity(session.email, `admin_callback_${parsed.action}`, updated.map(result => result.id).join(','), request);
    ...
    const message = error.message || 'Callback administrative action failed.';
